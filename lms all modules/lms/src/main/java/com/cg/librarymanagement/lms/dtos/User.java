...
public class User {
	...
	@NotEmpty
	@Pattern(regexp = "^(?=.*\\d).{4,8}$",message ="Password Should contain atleast one digit And length should be Min=4 & Max=10")
	private String password;
	
	@NotBlank
	@Size(min=2, max=20)
	@Pattern(regexp = "^[A-Za-z]*",message ="Name should contain only Albhabets")
	private String firstName;
	
	private String lastName;
	
	@Size(min=10,max=10,message = "Enter valid 10 digit phone number")
	@Pattern(regexp = "^[0-9]*",message ="Phone number can have only digits")
